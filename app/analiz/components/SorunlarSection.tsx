        ...
        {filteredSorunlar && filteredSorunlar.length > 0 ? (
          <div className="grid grid-cols-1 gap-3 sm:gap-4">
            {filteredSorunlar.map((sorun, index) => (
              <div key={index} className="bg-dark-300 rounded-lg p-3 sm:p-4 border border-gray-600 hover:border-primary transition-all duration-200">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`px-2 py-0.5 sm:py-1 rounded-full text-xs font-medium ${
                    ...
                    {sorun.severity === 'high' ? (t?.issues?.highRisk || 'Yüksek Risk') :
                     sorun.severity === 'medium' ? (t?.issues?.mediumRisk || 'Orta Risk') :
                     (t?.issues?.lowRisk || 'Düşük Risk')}
                  </span>
                </div>
