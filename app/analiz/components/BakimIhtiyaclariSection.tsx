  ...
  return (
    <div className="bg-dark-200 p-8 rounded-xl border border-gray-700 shadow-lg">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-2xl font-medium text-white">{t?.maintenance?.title || 'Bakım İhtiyaçları'}</h3>
        
        {/* Bakım ihtiyaçları sayaçları */}
        <div className="flex items-center space-x-3">
          {maintenanceNeeds?.immediate && maintenanceNeeds.immediate.length > 0 && (
            <div className="flex items-center px-3 py-1.5 bg-red-500/10 rounded-lg border border-red-500/30">
              <span className="text-red-500 font-medium">{maintenanceNeeds.immediate.length}</span>
              <span className="text-gray-300 ml-1.5">Acil</span>
            </div>
          )}
          
          {maintenanceNeeds?.soon && maintenanceNeeds.soon.length > 0 && (
            <div className="flex items-center px-3 py-1.5 bg-yellow-500/10 rounded-lg border border-yellow-500/30">
              <span className="text-yellow-500 font-medium">{maintenanceNeeds.soon.length}</span>
              <span className="text-gray-300 ml-1.5">Yakında</span>
            </div>
          )}
          
          {maintenanceNeeds?.future && maintenanceNeeds.future.length > 0 && (
            <div className="flex items-center px-3 py-1.5 bg-green-500/10 rounded-lg border border-green-500/30">
              <span className="text-green-500 font-medium">{maintenanceNeeds.future.length}</span>
              <span className="text-gray-300 ml-1.5">İleride</span>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-red-500/10 p-4 rounded-lg border border-red-500/30">
            ...
            <ul className="space-y-3">
              {maintenanceNeeds.immediate.map((need, index) => (
                <li key={index} className="bg-dark-400/40 p-3 rounded-lg">
         ...
         (!maintenanceNeeds?.future || maintenanceNeeds.future.length === 0) && (
          <p className="text-green-500 text-center bg-green-500/10 p-4 rounded-lg border border-green-500/30">
            <span className="flex items-center justify-center space-x-2">
              ...
              <span>Acil bakım gerektiren bir durum bulunmamaktadır.</span>
            </span>
          </p>
        )}
      </div>
